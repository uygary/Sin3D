...
/// </summary>
public class ForwardPlusRenderer3D
{
    private readonly GraphicsDevice _graphicsDevice;
    private readonly bool _useCrr;
    private readonly bool _cull;

    private readonly TileFrustumCuller _lightCuller;
    private readonly float[] _tileIndexBuffer;
    
    // Double-buffered for stereo VR: each eye gets its own texture so that
    // the second eye's SetData doesn't corrupt the first eye's in-flight GPU reads.
    private readonly Texture2D[] _lightGridTextures;
    ...
    /// <summary>
    /// Creates a new <see cref="ForwardPlusRenderer3D"/> object.
    /// </summary>
    ...
    public ForwardPlusRenderer3D(GraphicsDevice graphicsDevice, bool useCrr, bool cull)
    {
        _graphicsDevice = graphicsDevice;
        _useCrr = useCrr;
        _cull = cull;
        
        // Foundation for Forward+ tile grid mapping (16x16 tiles scaling up to 1080p equivalent view)
        _tileCountX = 120;
        _tileCountY = 68;
        _maxLightsPerTile = 32; 
        
        _lightCuller = new TileFrustumCuller(_tileCountX, _tileCountY, _maxLightsPerTile);
        _tileIndexBuffer = new float[_tileCountX * _tileCountY * _maxLightsPerTile];

        // Double-buffer: index 0 for first eye, index 1 for second eye
        var totalTiles = _tileCountX * _tileCountY;
        _lightGridTextures =
        [
            new Texture2D(_graphicsDevice, _maxLightsPerTile, totalTiles, false, SurfaceFormat.Single),
            new Texture2D(_graphicsDevice, _maxLightsPerTile, totalTiles, false, SurfaceFormat.Single)
        ];
        _lightDataTextures =
        [
            new Texture2D(_graphicsDevice, 4096, 2, false, SurfaceFormat.Vector4),
            new Texture2D(_graphicsDevice, 4096, 2, false, SurfaceFormat.Vector4)
        ];
        _lightDataBuffer = new Vector4[4096 * 2];

        _crrLightPositions = new Vector3[4096];
        _pointLightRadii = new float[4096];

        ResetRenderingSettings();
    }

    /// <summary>
    /// Evaluates the active scene lights into tile clusters before rendering.
    /// To be called exactly once per frame (or once per eye in VR) prior to drawing models.
    /// </summary>
    public void PrepareLights<TCamera>(TCamera camera,
        Vector4[] crrLightPositionsAndRadii,
        Vector3[] lightColors,
        int activeLightCount)
        where TCamera : ICamera
    {
        // Build CRR-adjusted view matrix for screen-space projection.
        // The incoming light positions are already CRR (world minus cameraPos),
        // so the view matrix must also have its translation cleared to match.
        ...
        // Extract CRR positions and radii into separate arrays for the culler
        for (var i = 0; i < activeLightCount; i++)
        {
            _crrLightPositions[i] = new Vector3(
                crrLightPositionsAndRadii[i].X,
                crrLightPositionsAndRadii[i].Y,
                crrLightPositionsAndRadii[i].Z);
            _pointLightRadii[i] = crrLightPositionsAndRadii[i].W;

            _lightDataBuffer[i] = crrLightPositionsAndRadii[i];
            _lightDataBuffer[4096 + i] = new Vector4(lightColors[i], 1f);
        }

        // Project lights to screen-space tiles
        _lightCuller.CullLights(
            in viewMatrix,
            in projectionMatrix,
            in _crrLightPositions,
            _pointLightRadii,
            activeLightCount,
            _tileIndexBuffer,
        ...
        where TCamera : ICamera
    {
        ...
        //drawing
        for (var i = 0; i < model.BaseModel.Meshes.Count; i++)
        {
        ...
        where TCamera : ICamera
    {
        ...
        var vieMatrix = camera.ViewMatrix;
        if (_useCrr)
        {
            vieMatrix.M41 = 0;
            vieMatrix.M42 = 0;
        ...
        effect.Parameters["View"]?.SetValue(vieMatrix);
        ...
        where TCamera : ICamera
    {
        ...
        for (var i = 0; i < model.BaseModel.Meshes.Count; i++)
        {
            var mesh = model.BaseModel.Meshes[i];
